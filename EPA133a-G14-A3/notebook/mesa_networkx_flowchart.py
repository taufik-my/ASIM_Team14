...
#  TITLE
# ====================================================================
ax.text(CX, 19.55, "MESA - NetworkX Data Exchange", ha="center",
        va="center", fontsize=16, fontfamily=FONT, fontweight="bold",
        color="#1B2631")
ax.text(CX, 19.2, "Bangladesh Road Transport Simulation (EPA133a - Group 14)",
        ha="center", va="center", fontsize=9, fontfamily=FONT,
        color="#777", fontstyle="italic")
ax.plot([2.5, 11.5], [19.0, 19.0], color="#CCD1D1", lw=0.7)

# ====================================================================
#  1  DATA INPUT
# ====================================================================
phase_dot(18.65, 1, "Data Input", "#2471A3")

_, b_csv, t_csv = rbox(COL_L, 17.7, BW, 0.8,
    "network_data.csv\nroad | id | type | lat | lon | length | condition | lrp",
    C_INPUT, fs=7.5, bold=True, bc="#85C1E9")

_, b_par, t_par = rbox(COL_R, 17.7, BW, 0.8,
...
#  2  MODEL INIT
# ====================================================================
phase_dot(17.15, 2, "Model Init", "#7D3C98")

_, b_init, t_init = rbox(CX, 16.3, BW_W, 0.75,
...
#  3  GENERATE MODEL
# ====================================================================
phase_dot(15.85, 3, "Generate Model", "#7D3C98")

_, b_gen, t_gen = rbox(CX, 15.1, 7.0, 0.6,
...
# ── domain zones ────────────────────────────────────────────────────
zone(0.6, 12.6, 6.0, 2.3, "#FFF5E6", "MESA Domain", "#E67E22")
zone(7.4, 12.6, 6.0, 2.3, "#EAF7ED", "NetworkX Domain", "#27AE60")

_, b_ag, t_ag = rbox(COL_L, 12.8, 5.2, 1.8,
...
_, b_gr, t_gr = rbox(COL_R, 12.8, 5.2, 1.8,
...
#  4  SIMULATION LOOP
# ====================================================================
phase_dot(12.15, 4, "Simulation Loop (7,200 steps)", "#2C3E50")

zone(0.6, 3.0, 12.8, 8.9, "#F8F9F9", "", "#AAB7B8")

_, b_st, t_st = rbox(CX, 11.05, BW_W, 0.7,
...
# ── Source ──────────────────────────────────────────────────────────
_, b_src, t_src = rbox(COL_L, 9.7, BW, 0.8,
...
_, b_rt, t_rt = rbox(CX, 8.0, BW_W, 1.1,
    "get_route( source_id )\n\n"
    "Pick random sink  -->  nx.shortest_path(G, source, sink, weight='length')\n"
...
# ── Bridge ──────────────────────────────────────────────────────────
_, b_br, t_br = rbox(COL_R, 6.2, BW, 1.1,
    "Bridge Interaction\n\n"
...
harrow(COL_L + BW / 2, COL_R - BW / 2, 6.75, "bridge?")

# ── Sink ────────────────────────────────────────────────────────────
_, b_sk, t_sk = rbox(CX, 3.6, BW_W, 1.3,
...
#  5  DATA OUTPUT
# ====================================================================
phase_dot(2.9, 5, "Data Output", "#C0392B")

_, b_ex, t_ex = rbox(COL_L, 1.8, BW, 0.7,
    "model.export_data()\n"
...
_, b_co, t_co = rbox(COL_L, 0.7, BW, 0.7,
...
_, b_an, t_an = rbox(COL_R, 0.7, BW, 0.7,
    "experiment_analysis.ipynb\n"
    "Compare scenarios & visualise",
    C_OUTPUT, fs=7.5, bc="#EC7063")

harrow(COL_L + BW / 2, COL_R - BW / 2, 1.05, "read CSVs")
